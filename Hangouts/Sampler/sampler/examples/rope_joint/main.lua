...
createRope = function( ropeGroup, x, y )	
	-- Create a block with a dynamic body
	--
	local blockA = display.newRect( ropeGroup, 0, 0, 120, 80 )
	blockA.x = x
	blockA.y = y
	...
	-- Create a block with a dynamic body
	--
	local blockB = display.newRect( ropeGroup, 0, 0, 120, 80 )
	blockB.x = x
	blockB.y = y + 300
	...
	--
	helpers.addNameLabel( ropeGroup, blockA, "Rope A")
	helpers.addNameLabel( ropeGroup, blockB, "Rope B")
...
local blockA, blockB = createRope( group, display.contentCenterX, display.contentCenterY)
