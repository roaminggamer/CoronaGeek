...
function public.createPlayer( reticle )
	----[[
	local player = greenArcherMaker.create( layers.content, centerX, centerY, 1.25 )
	physics.addBody( player, "dynamic", { radius = 25, filter = myCC:getCollisionFilter( "player" ) }  )
	player.colliderName = "player"
	player:playAngleAnim( "paused", normRot( player.rotation ) )
	...
	function player.enterFrame( self, event )		
		...
		player:playAngleAnim( self.baseAnim, normRot( player.rotation ) )
...
function public.createBack()
	...
	-- Pretty image for demo
	newImageRect( layers.underlay, centerX, centerY, "images/LostGardenBack.png", { w = 570 * 2, h = 380 * 2} )
...
function public.createHUDs()
	...
	easyIFC:quickLabel( layers.interfaces, "Zombies + Mistake", left + 10, top + 50, gameFont, 24, _W_, 0)
