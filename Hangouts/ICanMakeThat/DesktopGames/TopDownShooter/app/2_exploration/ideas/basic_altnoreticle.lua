...
function public.createPlayer()
	...
	function player.mouse( self, event )
		...
		if( self.lastMouseFrame == curFrame ) then return false end
		local df = curFrame - self.lastMouseFrame
		self.lastMouseFrame = curFrame

		if( self.firstMX ) then 
			if( event.x ~= self.lastMX or event.y ~= self.lastMY ) then

				local vec = diffVec( self.firstMX, self.firstMY, event.x, event.y, true )
				...
				if( len > maxDist ) then
					vec = normVec( vec )
					vec = scaleVec( vec, maxDist )
					self.firstMX = event.x - vec.x
					self.firstMY = event.y - vec.y
				elseif( len < minDist ) then
					vec = normVec( vec )
					vec = scaleVec( vec, minDist )
					self.firstMX = event.x - vec.x
					self.firstMY = event.y - vec.y
				end


				-- Show virtual 'joystick line'
				if( debugEn == true ) then
					if( self.lastLine ) then
						display.remove(self.lastDot)
						display.remove(self.lastLine)
					end
					self.lastDot = newCircle( self.parent, self.firstMX, self.firstMY, { size = 5, fill = _T_, stroke = _Y_, strokeWidth = 2 })
					self.lastLine = display.newLine( self.parent, self.firstMX, self.firstMY, event.x, event.y )
					self.lastLine.strokeWidth = 2
					self.lastLine:setStrokeColor(unpack(_Y_))
				end
			end
		else
			self.firstMX = event.x
			self.firstMY = event.y
		end
		self.lastMX = event.x
		self.lastMY = event.y
...
function public.createEnemies( player )
	...
	local enemy = newImageRect( layers.content, x, y, "images/enemy.png", 
				{ size = size, fill = _R_ },
				{ radius = size/2, bodyType = "dynamic", calculator = myCC, colliderName = "enemy" } )
...
function public.createBullet( barrel )
	...
	local vec = angle2Vector( barrel.rotation, true )
	...
	bullet.collision = function( self, event )
		...
		bullets[self] = nil
		enemies[other] = nil -- For now, we know ONLY enemies are being hit
...
function public.createHUDs()
	...
	easyIFC:quickLabel( layers.interfaces, "Basic no Reticle ", left + 10, top + 50, gameFont, 24, _W_, 0)
