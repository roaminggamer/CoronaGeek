...
function public.createPlayer()
	...
	function player.mouse( self, event )
		...
		if( self.lastMX ) then 
			local dx = event.x - self.lastMX
			local dy = event.y - self.lastMY

			self.rotation = self.rotation + dx * rotPerPixel

			normRot( self ) -- keep rotation in range [ 0, 360 )

			print( df, dx, self.rotation )
