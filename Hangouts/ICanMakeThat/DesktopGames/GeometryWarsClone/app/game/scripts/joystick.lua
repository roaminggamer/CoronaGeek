...
function joy.create( group, x, y, params )
	...
	if(inputObj) then
		...
		function joystick:touch( event )
			...
			if(percent == 0 ) then
				post(eventName, {angle=angle, vx=vx, vy=vy, x=nx, y=ny, percent=percent, phase = event.phase, state = "off" }, debugLevel)
			else
				post(eventName, {angle=angle, vx=vx, vy=vy, x=nx, y=ny, percent=percent, phase = event.phase, state = "on" }, debugLevel)
			end
