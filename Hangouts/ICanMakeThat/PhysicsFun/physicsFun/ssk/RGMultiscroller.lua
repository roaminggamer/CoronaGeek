...
onHScrollTouch = function( self, event  )   
    ...
    if( event.phase == "moved" ) then
        ...
        if( self.isFocus ~= true ) then
            self.isFocus = true
            display.currentStage:setFocus( self, event.id ) -- EDOCHI
        end
    ...
    elseif(event.phase == "ended" or event.phase == "cancelled") then
        ...
        dragLayer._dragging = false
        self.isFocus = false
        display.currentStage:setFocus( self, nil ) -- EDOCHI
        timer.performWithDelay( 1, function() dragMode = "click" end )
...
--EDOCHI    if(throwing) then return false end -- Stay out while throwing EFM
    --if(dragMode ~= "horiz") then return false end
