...
function scene:create( event )
	...
	local tmp = easyIFC:quickLabel( layers.overlay, "Posable Rag Doll", centerX, top + 20, gameFont2, 22, hexcolor("#2B547E")  )
	...
	easyIFC:presetPush( layers.overlay, "default", right - 35, top + 55, 60, 30, "Save", onSave, { labelSize = 12 } )
	easyIFC:presetPush( layers.overlay, "default", right - 35, top + 90, 60, 30, "Restore", onRestore, { labelSize = 12 } )
end
...
onSave = function( event )
	...
	for k,v in pairs( parts ) do
		pose[k] = { x = v.x, y = v.y, r = v.rotation }
	end
	table.save( pose, "pose.json")
	physics.start()
	--table.print_r(pose)
	return true
end
...
onRestore = function( event )
	local pose = table.load( "pose.json")
	if( not pose ) then return true end
	physics.pause()
	for k,v in pairs( pose ) do
		parts[k].x = pose[k].x
		parts[k].y = pose[k].y
		parts[k].rotation = pose[k].r
	end
	physics.start()
	--table.print_r(pose)
	return true
