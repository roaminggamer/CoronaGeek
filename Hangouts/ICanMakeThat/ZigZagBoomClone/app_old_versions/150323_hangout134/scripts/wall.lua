...
function public.newSegment( length )	
	...
	-- Create a score 'trigger'
	--
	local scoreTrigger = display.newRect( layers.content2, vx, vy, pathWidth, 10 )
	scoreTrigger.alpha = 00.1
	scoreTrigger.isTrigger = true
	
	-- Add a body to the 'trigger'
	--
	physics.addBody( scoreTrigger, "static" )
	...
	if( wallSegments and printSegmentCount ) then		
		...
		wallSegments[scoreTrigger] = scoreTrigger
