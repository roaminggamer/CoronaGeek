...
-- ==
local function fixedCamera( trackObj, layers, params )	
	...
	layers.world.enterFrame = function( event )
		if( not isDisplayObject( layers ) ) then return end
		if( not isDisplayObject( trackObj ) ) then return end
		local dx = 0
		local dy = 0
		--local dr = 0
		if(not lockX) then dx = trackObj.x - lx end		
		if(not lockY) then dy = trackObj.y - ly end
		--if( trackRot ) then dr = trackObj.rotation - lr end
		...
		--if( trackRot and dr ) then 
			--layers.world:rotate( dr )
			--lr = trackObj.rotation
		--end
...
-- ==
local function looseCamera( trackObj, boundarySize, layers, params )	
	if( not isDisplayObject( layers ) ) then return end
...
-- ==
local function looseRadialCamera( trackObj, deadRadius, bufferSize, layers, params )	
	if( not isDisplayObject( layers ) ) then return end
...
local function looseCamera_orig( trackObj, boundarySize )	
