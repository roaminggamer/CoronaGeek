...
function builder.create( layers, data )
	...
	if( common.niceGraphics ) then
		aPiece = display.newImageRect( layers.content, "images/kenney/barnacle.png", common.monsterSize, common.monsterSize )
		aPiece.x = data.x
		aPiece.y = data.y
	else
		aPiece = display.newCircle( layers.content, data.x, data.y, common.monsterSize/2 )
	...
	--
	local base = common.blockSize/2 + common.monsterSize/2 - 5
	local offset = {}
	offset[1] = { 0, -base, 0 }
	offset[2] = { base, 0, 90 }
	offset[3] = { 0, base, 180}
	...
	physics.addBody( aPiece, "static", 
		             { density = 1, bounce = 0, friction = 1, radius = common.monsterSize/2,
		               filter = common.myCC:getCollisionFilter( "monster" ) } )

	-- Tip: Make sure monster never overlay's platforms by pusing it to the back of the render layer it is in.
	aPiece:toBack()
