...
function builder.create( layers, data  )
	...
	if( common.niceGraphics ) then
		aPiece = display.newImageRect( layers.content, "images/kenney/particleHUD2.png", common.pickupSize, common.pickupSize )
		aPiece.x = data.x
		aPiece.y = data.y
	else
	...
	aPiece.collision = function( self, event )
		...
		Runtime:dispatchEvent( { name = "onPickup", pickupType = "decoy" } )
