...
function scene:create( event )
	...
	local back = display.newImageRect( screenGroup, "images/protoBack.png", 380, 570 )
	back.x = centerX
	back.y = centerY
	if(w>h) then back.rotation = 90 end
	
	local label = display.newText( screenGroup, "Scene 2:", centerX - 5, 20, native.systemFont, 20 )
	label.anchorX = 1
	label:setFillColor(0,1,0)

	timeLabel = display.newText( screenGroup, tostring(currentTime[timeIndex]), centerX + 5, 20, native.systemFont, 20 )
	timeLabel.anchorX = 0

	local tmp = display.newText( screenGroup, "Choose transition time:", 5, 40, native.systemFont, 10 )
	tmp.anchorX = 0
	...
	for i = 1, numButtons do
		...
		PushButton( screenGroup, sx + (i-1) * buttonSpacing, 60, 
			        tostring(currentTime[i]), 
			        onTimeChange,  
			        { labelColor = {1,1,1}, labelSize = 10, 
			          width = buttonWidth, height = 20 } )
	end	


	-- Add buttons for testing transitions
	local tmp = display.newText( screenGroup, "Click any button below to transition time:", 5, 90, native.systemFont, 10 )
	tmp.anchorX = 0
	...
	for i = 1, numButtons do
		...
		PushButton( screenGroup, centerX, sy + (i-1) * buttonSpacing, 
			        tostring(sceneTransitions[i]), 
			        onTimeChange,  
			        { labelColor = {1,1,1}, labelSize = 9, 
